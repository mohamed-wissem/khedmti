/**
 * BLACKFORGE gamification rules — single source of truth for XP, levels,
 * ranks, loyalty credit, daily rewards, and referral bonuses.
 */
...
export const dailyXp = (streak: number) => DAILY_BASE_XP + Math.min(Math.max(streak - 1, 0), 6) * 10;

export const REFERRAL_BONUS_CENTS = 500; // $5 credit to the referrer
...
export function evaluateClaim(now: Date, lastClaimAt: Date | null, streak: number): ClaimState {
  ...
  return { claimable: true, nextStreak: 1 }; // streak broken
}

// --- Achievements (derived from stats; no storage needed) ---
...
export function achievements(c: AchievementCtx): Achievement[] {
  return [
    { id: "first-blade", name: "First Blade", desc: "Place your first order", earned: c.orders >= 1 },
    { id: "stockpiler", name: "Stockpiler", desc: "Place 5 orders", earned: c.orders >= 5 },
    { id: "big-spender", name: "Big Spender", desc: "Spend $200 total", earned: c.spentCents >= 20000 },
    { id: "forge-loyalist", name: "Forge Loyalist", desc: "Reach a 7-day streak", earned: c.streak >= 7 },
